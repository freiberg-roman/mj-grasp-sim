...
XML = r"""
    <worldbody>
        <body name="{name}" pos="{position}" quat="{quaternion}">
            ...
            <geom name="geom:{name}" rgba="1.0 0.32 0.32 1" size="{size} {size} {size}" type="box" mass="1.0"/>
        </body>
    </worldbody>
"""


class ObjectCube(MjXml):
    def __init__(self, pose: SE3Pose, name: str, size: float):
